...
Guardian is a CLI tool that scaffolds token-optimized agent frameworks for AI-assisted development. It enables developers to set up deterministic, validated workflows across multiple AI tools with a single command.

### Pi-First Architecture

Guardian uses **pi as its native format and source of truth**. Other tool formats (`.claude/`, `.opencode/`, `.agents/`) are generated exports from the pi source.
...
**Implementation:** Copy templates from Rigorix `~/project/rigorix/.claude/` → `~/project/guardian-framework/templates/pi/`. Templates have placeholders that users fill in during scaffold.
...
**Package name:** `guardian-framework` (no scope)

| Scenario | Recommendation |
|----------|----------------|
| Private use | Works fine with unscoped name |
| Open source later | Can keep `guardian-framework` or migrate to `@guardian/cli` |
| Name collision risk | Low — `guardian-framework` not currently on npm |
...
| Distribution | npm | `npx guardian-framework` or `bunx guardian-framework` |
...
const readme = `# Guardian Framework (${tool})
...
Generated by Guardian v${manifest.frameworkVersion}
`;
...
**Usage:**
```bash
npx guardian-framework init [options]
```
...
3. User edits `.pi/` → regenerate exports with `guardian-framework generate`
...
**Usage:**
```bash
npx guardian-framework generate [options]
```
...
```
$ npx guardian-framework generate --dry-run
...
```
$ npx guardian-framework generate --tool claude
...
**Usage:**
```bash
npx guardian-framework update [options]
```
...
**Usage:**
```bash
npx guardian-framework upgrade <version> [options]
```
...
**Usage:**
```bash
npx guardian-framework info [options]
```
...
Commands:
  ...
  npx guardian-framework upgrade v1.1      # Version migration
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description: "Run Guardian validation scripts",
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (fs.existsSync(manifestPath)) {
      manifest = JSON.parse(fs.readFileSync(manifestPath, "utf-8"));
      ctx.ui.notify(`Guardian v${manifest.frameworkVersion} loaded`, "success");
    }
  });
  ...
  pi.registerCommand("guardian-info", {
    ...
    handler: async (args, ctx) => {
      if (!manifest) {
        ctx.ui.notify("No guardian-manifest.json found. Run guardian-framework init first.", "error");
        return;
      }
...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("guardian-update", {
    description: "Update Guardian framework with smart merge",
    handler: async (args, ctx) => {
...
| No manifest found (info) | Display "No Guardian framework detected" | 1 |
...
**Package:** `guardian-framework`

**Additional:** `@guardian/pi-framework` (optional pi package install)

**Installation:**
```bash
npx guardian-framework init                  # Scaffold
bunx guardian-framework init --tool pi       # Explicit pi
...
The following patterns from [Terax AI](https://github.com/crynta/terax-ai) have been adopted into Guardian templates:
