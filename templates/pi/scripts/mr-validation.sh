#!/usr/bin/env bash
# ============================================================================
# mr-validation.sh — Validate Merge Request/Pull Request
#
# Run as: bash .claude/scripts/mr-validation.sh [pr-number]
# Checks: CI status, architecture, security, integration
# ============================================================================
set -euo pipefail

PR_NUMBER="${1:-}"

RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

ERRORS=()
PASS_COUNT=0

pass() { echo -e "${GREEN}✅ PASS${NC} $1"; ((PASS_COUNT++)); }
fail() { echo -e "${RED}❌ FAIL${NC} $1"; ERRORS+=("$1"); }
warn() { echo -e "${YELLOW}⚠️  WARN${NC} $1"; }

echo "============================================"
echo "  MR Validation"
echo "============================================"
echo ""

# ---------------------------------------------------------------------------
# Check GitHub CLI
# ---------------------------------------------------------------------------
if ! command -v gh &> /dev/null; then
    echo -e "${RED}❌ GitHub CLI (gh) not installed${NC}"
    exit 1
fi

# ---------------------------------------------------------------------------
# Get PR Number
# ---------------------------------------------------------------------------
if [ -z "$PR_NUMBER" ]; then
    # Try to get from current branch
    BRANCH_NAME=$(git branch --show-current)
    PR_NUMBER=$(gh pr view --json number -q '.number' 2>/dev/null || echo "")

    if [ -z "$PR_NUMBER" ]; then
        echo -e "${RED}❌ PR number required${NC}"
        echo "Usage: mr-validation.sh [pr-number]"
        echo "  Or run from PR branch"
        exit 1
    fi
fi

echo "Validating PR #$PR_NUMBER..."

# ---------------------------------------------------------------------------
# Get PR Status
# ---------------------------------------------------------------------------
PR_INFO=$(gh pr view "$PR_NUMBER" --json title,state,mergeable,statusCheckRollup,headRefName 2>/dev/null || echo "{}")
PR_STATE=$(echo "$PR_INFO" | jq -r '.state // "unknown"')
PR_MERGEABLE=$(echo "$PR_INFO" | jq -r '.mergeable // "unknown"')
PR_BRANCH=$(echo "$PR_INFO" | jq -r '.headRefName // "unknown"')

echo "Title: $(echo "$PR_INFO" | jq -r '.title')"
echo "State: $PR_STATE"
echo "Branch: $PR_BRANCH"
echo ""

# ---------------------------------------------------------------------------
# CI Status Check
# ---------------------------------------------------------------------------
echo "--- CI Status ---"

STATUS_CHECKS=$(echo "$PR_INFO" | jq -r '.statusCheckRollup[]?.status // "unknown"' 2>/dev/null || echo "unknown")

if [ "$STATUS_CHECKS" = "SUCCESS" ] || [ "$STATUS_CHECKS" = "success" ]; then
    pass "CI checks passed"
elif [ "$STATUS_CHECKS" = "IN_PROGRESS" ] || [ "$STATUS_CHECKS" = "pending" ]; then
    warn "CI checks in progress"
    echo "  Wait for completion and re-run"
else
    fail "CI checks failed or unknown: $STATUS_CHECKS"
fi

# ---------------------------------------------------------------------------
# Mergeable Check
# ---------------------------------------------------------------------------
echo ""
echo "--- Mergeable Status ---"

if [ "$PR_MERGEABLE" = "MERGEABLE" ] || [ "$PR_MERGEABLE" = "true" ]; then
    pass "PR is mergeable"
elif [ "$PR_MERGEABLE" = "CONFLICTING" ] || [ "$PR_MERGEABLE" = "false" ]; then
    fail "PR has merge conflicts"
    echo "  Resolve conflicts before merge"
else
    warn "Mergeable status unknown: $PR_MERGEABLE"
fi

# ---------------------------------------------------------------------------
# Run Local Validators
# ---------------------------------------------------------------------------
echo ""
echo "--- Local Validation ---"

# Checkout PR branch
git checkout "$PR_BRANCH" 2>/dev/null || {
    warn "Could not checkout PR branch"
}

# Run validation scripts
if [ -f ".claude/scripts/validate-ci.sh" ]; then
    bash .claude/scripts/validate-ci.sh 2>&1 | tail -5
fi

if [ -f ".claude/scripts/validate-security.sh" ]; then
    bash .claude/scripts/validate-security.sh 2>&1 | tail -5
fi

# ---------------------------------------------------------------------------
# Architecture Check (if validator exists)
# ---------------------------------------------------------------------------
echo ""
echo "--- Architecture Check ---"

# Check for wiring issues
CHANGED_FILES=$(gh pr diff "$PR_NUMBER" --name-only 2>/dev/null || git diff --name-only "origin/main"..HEAD)

for FILE in $CHANGED_FILES; do
    if [ -f "$FILE" ] && [[ "$FILE" == *.rs ]]; then
        # Check for new public functions
        NEW_FNS=$(git diff "origin/main"..HEAD -- "$FILE" | grep "^[+]pub fn\|^[+]pub async fn" || true)
        if [ -n "$NEW_FNS" ]; then
            echo -e "${BLUE}ℹ️  New public functions in $FILE:${NC}"
            echo "$NEW_FNS" | head -5
        fi
    fi
done

# ---------------------------------------------------------------------------
# Check for Tests
# ---------------------------------------------------------------------------
echo ""
echo "--- Test Coverage Check ---"

TEST_FILES=$(echo "$CHANGED_FILES" | grep -E "test.*\.rs|\.test\.rs|tests/" || true)
SRC_FILES=$(echo "$CHANGED_FILES" | grep -E "src/.*\.rs|rigorix/src/.*\.rs" | grep -v test || true)

if [ -n "$SRC_FILES" ] && [ -z "$TEST_FILES" ]; then
    warn "Source files changed but no test files"
    echo "  Consider adding tests for:"
    echo "$SRC_FILES" | head -5
else
    pass "Test coverage present"
fi

# ---------------------------------------------------------------------------
# Summary
# ---------------------------------------------------------------------------
echo ""
echo "============================================"
echo "  Summary"
echo "============================================"
echo -e "  Passed:   ${GREEN}${PASS_COUNT}${NC}"
echo -e "  Failed:   ${RED}${#ERRORS[@]}${NC}"
echo ""

if [ ${#ERRORS[@]} -gt 0 ]; then
    echo "BLOCKERS:"
    for err in "${ERRORS[@]}"; do
        echo "  - $err"
    done
    echo ""
    echo "Fix blockers before merging"
    exit 1
fi

echo -e "${GREEN}MR validation passed. Ready for merge.${NC}"
echo ""
echo "Merge with: gh pr merge $PR_NUMBER --squash --delete-branch"

exit 0