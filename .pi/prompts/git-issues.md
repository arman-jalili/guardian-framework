...
# For GitHub (gh)
gh auth status
gh repo view arman-jalili/guardian-framework

# For GitLab (glab)
glab auth status
glab repo view arman-jalili/guardian-framework
```
...
```bash
# Create milestone
gh api repos/arman-jalili/guardian-framework/milestones \
  -f title="[EPIC_NAME]" \
  -f description="[EPIC_DESCRIPTION]" \
  -f state="open"

# Get milestone number
gh api repos/arman-jalili/guardian-framework/milestones --jq '.[] | select(.title=="[EPIC_NAME]") | .number'
```
...
| Repository not found | Check `arman-jalili/guardian-framework` format (owner/repo) |
